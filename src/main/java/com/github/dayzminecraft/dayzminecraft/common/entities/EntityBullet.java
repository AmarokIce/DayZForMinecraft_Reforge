...
public class EntityBullet extends EntityThrowable {
    ...
    public void setVelocity(double motionX, double motionY, double motionZ) {
        ...
        if (prevRotationPitch == 0.0F && prevRotationYaw == 0.0F) {
            ...
            prevRotationYaw = (float) ((Math.atan2(motionX, motionZ) * 180D) / Math.PI);
            prevRotationPitch = (float) ((Math.atan2(motionY, f) * 180D) / Math.PI);
        ...
        if (movingObjectPosition.entityHit != null) {
            ...
            if (movingObjectPosition.entityHit instanceof EntityPlayer) {
                if (worldObj.difficultySetting.equals(EnumDifficulty.EASY)) {
                    ...
                    if (j == 0)
                        ((EntityLivingBase) movingObjectPosition.entityHit).addPotionEffect(new PotionEffect(Effect.bleeding.getId(), 20 * 300, 1));
